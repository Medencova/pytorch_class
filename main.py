...
trainset = CellsDataset(data=getdata('./train.txt'), transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=16, shuffle=True, pin_memory=True, num_workers=2)

testset = CellsDataset(data=getdata('./test.txt'), transform=transform_test)
...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/ckpt.t7.2')
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt.t7.2')
        best_acc = acc
        ...
        for image in range(0, length):
            ...
            for batch in range(0, count):
                if not dict:
                    ...
                    continue
                if big_predict[batch][image in dict:
                    dict[big_predict[batch][image]] += 1
                else:
                    dict[big_predict[batch][image]] = 1
